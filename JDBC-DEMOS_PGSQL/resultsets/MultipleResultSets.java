...
public class MultipleResultSets {
    ...
    final static String cmdIns = "insert into jdbcdemo select max(id)+1,'Teste Multiplos Comandos' from jdbcdemo; " +
            "select * from jdbcdemo;";
	public static void main(String[] args) throws SQLException {
		...
		try {
			...
			boolean res = stmt.execute(cmdIns);
            if (!res) stmt.getMoreResults(); // first is not a resultSet
